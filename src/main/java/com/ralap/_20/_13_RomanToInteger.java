...
public class _13_RomanToInteger {
    ...
    public int solution(String roman) {
        ...
        while (currIndex < roman.length()) {
            if (currIndex == romanLen - 1) {
                num += map.get(roman.substring(currIndex, currIndex + 1)).intValue();
                break;
            } else {
                String doubleChar = roman.substring(currIndex, currIndex + 2);
                if (map.containsKey(doubleChar)) {
                    num += map.get(doubleChar).intValue();
                    currIndex += 2;
                } else {
                    num += map.get(roman.substring(currIndex, currIndex + 1)).intValue();
                    currIndex++;
    ...
    public int solution_2(String roman) {
